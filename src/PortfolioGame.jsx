import React, { useState, useEffect, useRef } from 'react';
import { X } from 'lucide-react';

const INTERACTIVE_OBJECTS = [
  {
    id: 'desk',
    top: 5,
    left: 5,
    width: 3,
    height: 2,
    coordinate : [],
    emoji: '💻',
    title: 'Frontend Developer',
    company: 'Tech Corp',
    period: '2023 - Present',
    description: 'Built responsive web applications using React and TypeScript. Improved performance by 40%.',
    skills: ['React', 'TypeScript', 'Tailwind CSS']
  },
  {
    id: 'bookshelf',
    top: 1,
    left: 1,
    width: 1,
    height: 2,
    coordinate : [],
    emoji: '📚',
    title: 'Junior Developer',
    company: 'StartUp Inc',
    period: '2022 - 2023',
    description: 'Developed features for internal CMS with Excel import/export functionality.',
    skills: ['JavaScript', 'Node.js', 'Excel Integration']
  },
  {
    id: 'plant',
    top: 8,
    left: 7,
    width: 1,
    height: 1,
    coordinate : [],
    emoji: '🌱',
    title: 'Intern',
    company: 'Design Studio',
    period: '2021 - 2022',
    description: 'Assisted in building marketing websites and learned modern web development practices.',
    skills: ['HTML', 'CSS', 'JavaScript']
  }
];


INTERACTIVE_OBJECTS.forEach(obj => {
  for (let row = 0; row < obj.height; row++) {
    for (let col = 0; col < obj.width; col++) {
      obj.coordinate.push(`${obj.left + col},${obj.top + row}`);
    }
  }
});

console.log('check:', INTERACTIVE_OBJECTS);

function PortfolioGame() {
  const [selectedObject, setSelectedObject] = useState(null);
  const [nearbyObject, setNearbyObject] = useState(null);
  
  const BLOCK_SIZE = 50;
  const [playerPos, setPlayerPos] = useState([8, 1]);

  const FIELD_WIDTH = 10;
  const FIELD_HEIGHT = 10;
  const FIELD_POSITION = [100, 500];

  function isPositionBlocked(x, y) {
    return INTERACTIVE_OBJECTS.some(obj => 
      obj.coordinate.includes(`${x},${y}`)
    );
  }

  function getNearbyObject(x, y) {
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        if (dx === 0 && dy === 0) continue;
        
        const nearX = x + dx;
        const nearY = y + dy;
        
        const foundObject = INTERACTIVE_OBJECTS.find(obj => 
          obj.coordinate.includes(`${nearX},${nearY}`)
        );
        
        if (foundObject) {
          return foundObject;
        }
      }
    }
    return null;
  }

  function handleKeyDown(e) {
    setPlayerPos(prev => {
      let newPos = [...prev];
      
      if (e.key.toLowerCase() === 'w') {
        newPos[1] = Math.max(0, prev[1] - 1);
      } else if (e.key.toLowerCase() === 's') {
        newPos[1] = Math.min(FIELD_HEIGHT - 1, prev[1] + 1);
      } else if (e.key.toLowerCase() === 'a') {
        newPos[0] = Math.max(0, prev[0] - 1);
      } else if (e.key.toLowerCase() === 'd') {
        newPos[0] = Math.min(FIELD_WIDTH - 1, prev[0] + 1);
      }

      if (isPositionBlocked(newPos[0], newPos[1])) {
        return prev;
      }

      const nearObj = getNearbyObject(newPos[0], newPos[1]);
      if (nearObj) {
        console.log('Player is near:', nearObj.title);
        setNearbyObject(nearObj);
      } else {
        setNearbyObject(null);
      }
      
      return newPos;
    });
  }

  useEffect(() => {
    window.addEventListener('keydown', handleKeyDown);
    
    return () => {
      window.removeEventListener('keydown', handleKeyDown);
    };
  }, []);

  return (
    <div className="relative w-full h-screen bg-gray-900 overflow-hidden">
      {/* Background grid */}
      <div className="absolute inset-0">
        {Array.from({ length: FIELD_HEIGHT }).map((_, row) =>
          Array.from({ length: FIELD_WIDTH }).map((_, col) => (
            <div
              key={`${row}-${col}`}
              className="absolute border border-gray-800"
              style={{
                left: FIELD_POSITION[1] + col * BLOCK_SIZE,
                top: FIELD_POSITION[0] + row * BLOCK_SIZE,
                width: BLOCK_SIZE,
                height: BLOCK_SIZE,
                backgroundColor: 'rgba(255, 255, 255, 0.05)',
              }}
            />
          ))
        )}
      </div>

      {/* Player */}
      <div 
        className="absolute bg-blue-500 flex items-center justify-center text-white font-bold transition-transform"
        style={{
          left: FIELD_POSITION[1] + playerPos[0] * BLOCK_SIZE,
          top: FIELD_POSITION[0] + playerPos[1] * BLOCK_SIZE,
          width:  BLOCK_SIZE,
          height: BLOCK_SIZE
        }}
      >
        P
      </div>

      {INTERACTIVE_OBJECTS.map(obj => (
        <div
          key={obj.id}
          className="absolute flex items-center justify-center text-4xl cursor-pointer bg-red-500"
          style={{
            left: FIELD_POSITION[1] + (obj.left * BLOCK_SIZE),
            top: FIELD_POSITION[0] + (obj.top * BLOCK_SIZE),
            width: obj.width * BLOCK_SIZE,
            height: obj.height * BLOCK_SIZE
          }}
        >
          {obj.emoji}
        </div>
      ))}

      {/* Show nearby object info */}
      {nearbyObject && (
        <div className="absolute top-4 left-4 bg-white p-4 rounded shadow-lg">
          <p className="font-bold">Press E to interact:</p>
          <p className="text-lg">{nearbyObject.emoji} {nearbyObject.title}</p>
          <p className="text-sm text-gray-600">{nearbyObject.company}</p>
        </div>
      )}
    </div>
  );
}

export default PortfolioGame;